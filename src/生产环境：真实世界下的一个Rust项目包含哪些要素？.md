...
如果你想强迫自己要撰写每个公共接口的文档，保持系统有良好的文档覆盖，那么可以使用 `![deny(missing_docs)]`。这样，任何时候只要你忘记撰写文档，都会产生编译错误。如果你觉得编译错误太严格，也可以用编译报警： `![warn(missing_docs]`。之前我们阅读过 bytes crate 的源码，可以再回过头来看看它的 [lib.rs 的开头](images/464856/lib.rs)。

...
1. 你可以写一段代码，在 main() 函数里生成一个字符串，然后通过 `std::thread::spawn` 创建一个线程，让 main() 函数所在的主线程和新的线程共享这个字符串么？提示：使用 [std::sync::Arc](https://doc.rust-lang.org/std/sync/struct.Arc.html)。
